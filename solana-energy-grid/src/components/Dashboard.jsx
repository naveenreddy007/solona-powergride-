...
const Dashboard = ({ navigateTo, updateAppState, connectedWallet }) => {
  ...
  // Share state with parent App component
  useEffect(() => {
    if (buildings.length > 0 && trades.length > 0 && !isInitializing) {
      updateAppState(buildings, trades, energyHistory);
    }
  }, [buildings, trades, energyHistory, isInitializing, updateAppState]);
  
  // Check for wallet connection status
  useEffect(() => {
    // If a wallet is connected, update the simulation settings
    if (connectedWallet) {
      setIsInitializing(false);
      console.log(`Dashboard connected to wallet: ${connectedWallet.name}`);
      
      // Update the buildings with the wallet information if needed
      if (buildings.length > 0) {
        const updatedBuildings = buildings.map(building => {
          // For demo purposes, associate the first building with the connected wallet
          if (building.id === 'B1' || building.id === buildings[0].id) {
            return {
              ...building,
              wallet: { publicKey: connectedWallet.publicKey },
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="stats-panel">
        ...
        {connectedWallet && (
          <div className="stat-card wallet-info">
            ...
            <div className="wallet-details">
              ...
              <div className="wallet-data">
                ...
                <div className="wallet-balance">{connectedWallet.balance?.toFixed(3) || '0.000'} SOL</div>
              </div>
            </div>
          </div>
        )}
